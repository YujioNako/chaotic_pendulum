...
unit_step=Math.round(unit_step); //减小计算量
...
function setSize() { //根据窗口大小重新调整
  ...
  base_len = min/(sum(length) + ball_radius[ball_radius.length - 1]);
...
function update(timestamp) {
  ...
  if(previous==null){
    ...
    if(timestamp-previous>100){
      //console.log(`WARNING:${timestamp-previous}`); 
      ...
      //console.log(String((Energy-start_E)/start_E)+" "+String( timestamp-start_T));
